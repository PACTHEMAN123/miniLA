...
void mem_store(uint32_t addr, uint32_t sz, uint32_t value) {
    uint32_t index = addr >> 2;
    uint32_t byte_offset = addr & 0b11;
    uint32_t half_offset = byte_offset >> 1;
    uint32_t bitmask = 0xffffffff;
    Log("Store");
    Log("Mem addr = %8.8x", addr);
...
MEM2WB MEM(EX2MEM ex_info) {
    ...
    if(ex_info.is_mem) {
        switch (ex_info.mem_op) {
            case MEM_SB:  mem_sz = 1; is_store = 1;  break;
            case MEM_SH:  mem_sz = 2; is_store = 1;  break;
            case MEM_SW:  mem_sz = 4; is_store = 1;  break;
            case MEM_LB:  mem_sz = 1; is_signed = 1; break;
            case MEM_LH:  mem_sz = 2; is_signed = 1; break;
            case MEM_LW:  mem_sz = 4; is_signed = 1; break;
            case MEM_LBU: mem_sz = 1; is_signed = 0; break;
            case MEM_LHU: mem_sz = 2; is_signed = 0; break; 
